...
function render()
{
    ...
    ctm = mult( ctm, translate( 1.25, 0.0, 0.0 ) );
    ...
    // teikna "tunglid"
    ctm = mult( ctm, rotateY( -rotDay ) );
    ctm = mult( ctm, rotateY( rotMoon ) );
    ctm = mult( ctm, translate( 1.5, 0.0, 0.0 ) );

    ctm = mult( ctm, scalem( 0.4, 0.4, 0.4 ) );
    gl.uniformMatrix4fv(matrixLoc, false, flatten(ctm));
    gl.drawArrays( gl.TRIANGLES, 0, numVertices );


    //teikna "Mars"
    ctm1 = mult( ctm1, scalem( 0.075, 0.075, 0.075 ) );
    ctm1 = mult( ctm1, rotateY( rotYearMars ) );
    ctm1 = mult( ctm1, translate( 12, 0.0, 0.0 ) );
    ctm1 = mult( ctm1, rotateZ( -earthTilt ) );
    ctm1 = mult( ctm1, rotateY( rotDay ) );
